...
class TestReplOps:
    ...
    def test_play_malformed(self):
        ...
        assert self.c.play(code) == {
            "problems": [
                "<no file>:1:18 Unexpected control character (0) in note/rest/name"
            ],
            "status": ["done", "error"]
        }
    ...
    def test_export_empty(self):
        assert self.c.export() == {
            "binary-data":
                b"MThd\x00\x00\x00\x06\x00\x00\x00\x01\x00\x80MTrk\x00\x00"
    ...
    def test_new_score(self):
        assert self.c.new_score() == {
            "status": ["done"]
        }

    def test_replay(self):
        assert self.c.replay() == {
            "status": ["done"]
        }

    def test_replay_with_start_end(self):
        assert self.c.replay(start="0:00", end="0:01") == {
            "status": ["done"]
        }
    ...
    def test_score_data_empty(self):
        assert self.c.score_data() == {
            "data":
                '{"aliases":{},"current-parts":[],"events":[],"global-attributes":{},"markers":{},"parts":{},"variables":{}}',
            "status": ["done"]
        }
    ...
    def test_score_events(self):
        ...
        assert self.c.score_events() == {
            "events":
                '[{"type":"part-declaration","value":{"names":["piano"]}},{"attribute":"octave","type":"attribute-update","value":3},{"type":"note","value":{"duration":{"components":[{"denominator":2,"dots":0}]},"pitch":{"accidentals":[],"letter":"C"}}},{"type":"note","value":{"duration":{"components":[{"denominator":2,"dots":0}]},"pitch":{"accidentals":[],"letter":"D"}}},{"type":"note","value":{"duration":{"components":[{"denominator":2,"dots":0}]},"pitch":{"accidentals":[],"letter":"E"}}}]',
            "status": ["done"]
        }

    def test_score_text_empty(self):
        assert self.c.score_text() == {
            "text": "",
            "status": ["done"]
        }
